...
| `contentTheme` | `'default' \| 'gradient-card'` | `'default'` | Preset content visual theme. |
...
| `contentTheme` | `'default' \| 'gradient-card'` | `'default'` | 预设内容外壳的视觉主题。 |
...
1. Repeated center popup card skins should be exposed through `contentTheme`, such as `gradient-card`, instead of duplicating the same card shell in page SCSS.
1. 多处重复使用的居中弹窗卡片皮肤应通过 `contentTheme` 暴露，例如 `gradient-card`，不要在页面 SCSS 中重复写同一套外壳。
