...
test('home page delegates data orchestration and card rendering to focused modules', async () => {
    ...
    assert.match(orderCard, /className="home-page__order-amount flex items-center word-break mt-40"/)
    assert.match(orderCard, /className="home-page__order-progress mt-40"[\s\S]*className="home-page__order-progress-copy flex items-center flex-wrap gap-10 word-break size-24"[\s\S]*className="home-page__order-progress-bar mt-40"/)
    assert.match(orderCard, /t\('进度\(\{\{multiple\}\}x\)', \{ multiple: order\.progressMultipleText \}\)/)
    assert.doesNotMatch(orderCard, /进度\(\{\{multiple\}\}倍\)/)
    ...
    assert.match(styles, /&__order-progress\s*\{[\s\S]*width:\s*100%;/)
    assert.match(styles, /&__order-progress-copy/)
    assert.doesNotMatch(styles, /&__order-progress\s*\{[\s\S]*width:\s*200px;/)
