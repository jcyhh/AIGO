...
test('picker wraps popup and swiper into a reusable mobile picker', async () => {
    ...
    assert.doesNotMatch(showcasePage, /emptyText=/)
    assert.doesNotMatch(readme, /emptyText/)
    ...
    assert.match(showcasePage, /const tokenTone = String\(option\.tone/)
    assert.match(showcasePage, /picker-showcase-token-icon--\$\{tokenTone\}/)
    assert.doesNotMatch(showcasePage, /style=/)
    assert.match(showcaseStyle, /&--usdt/)
