...
test('home reward detail page fetches paged reward logs with reusable list components', async () => {
    ...
    assert.match(page, /createLatestRequestGuard: createRewardLogRequestGuard/)
    assert.match(page, /invalidateLatestRequest: invalidateRewardLogRequest/)
    assert.doesNotMatch(page, /rewardLogRequestRef/)
    assert.match(page, /const loadRewardLogs = useCallback\(async \([\s\S]*filter: HomeRewardDetailFilter,[\s\S]*nextPageNo: number,[\s\S]*\) => \{[\s\S]*const isCurrent = createRewardLogRequestGuard\(\)/)
    assert.match(page, /const refreshRewardLogs = useCallback\(async \(\) => \{[\s\S]*setPageNo\(1\)[\s\S]*setRewardLogs\(\[\]\)[\s\S]*setHasNextPage\(false\)[\s\S]*await loadRewardLogs\(activeFilter, 1\)[\s\S]*\}, \[activeFilter, loadRewardLogs\]\)/)
    assert.match(page, /const handlePagePullRefresh = useCallback\(\(\) => refreshRewardLogs\(\), \[refreshRewardLogs\]\)/)
    assert.match(page, /usePageRefresh\(handlePagePullRefresh, !loading\)/)
    assert.match(page, /return invalidateRewardLogRequest/)
    assert.match(page, /getOrderRewardLogs\(createHomeRewardDetailListParams\(filter, nextPageNo\)\)/)
    assert.match(page, /setRewardLogs\(\(current\) => nextPageNo === 1 \? response\.reward_logs : current\.concat\(response\.reward_logs\)\)/)
    assert.match(page, /setHasNextPage\(response\.reward_logs\.length >= HOME_REWARD_DETAIL_PAGE_SIZE\)/)
    assert.match(page, /useEffect\(\(\) => \{[\s\S]*if \(pageNo === 1\) return undefined[\s\S]*void loadRewardLogs\(activeFilter, pageNo\)[\s\S]*\}, \[activeFilter, loadRewardLogs, pageNo\]\)/)
